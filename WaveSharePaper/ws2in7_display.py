...
import logging
from WaveSharePaper.epd2in7_V2 import epd2in7_V2
import time
...
class WaveShare2In7Display():
    def __init__(self):
        epd = epd2in7_V2.EPD()

        logging.info("Init display and clear...")
        epd.init()
        epd.Clear()
        time.sleep(1)
    ...
    def get_canvas(self):
        frame = Image.new('1', (epd.width, epd.height), 255)  # 255: clear the frame
        self.canvas = ImageDraw.Draw(frame)
        return self.canvas

    def display(self):
        epd.display_Fast(epd.getbuffer(self.canvas))
